...
5. `/cost/`는 실제 비용 하위 URL이 112개인데 title·H1·description은 “100개 업종”이라고 썼다. 최초 파일럿에서는 title/H1 보호 원칙에 따라 보류했고, 이후 소유자 승인 후 고정 숫자 대신 실제 목록 길이(`LIST.length`)를 사용하도록 정정했다.
