...
const KEEP_LANDING_SLUGS = new Set([
  ...
  'academy-shopping-mall', 'cafe-app-dev-out', 'restaurant-website-dev', 'gym-web-dev',
]);
...
export const REDIRECTED_LANDING_SLUGS = new Map([
  ...
  // 성남의 구형 /l/ 서비스 랜딩은 같은 질문을 다루는 현행 지역 서비스 URL로 통합한다.
  ['seongnam-app-dev', '/app-development/seongnam/'],
  ['seongnam-app-dev-out', '/app-development/seongnam/'],
  ['seongnam-app-dev-cost', '/app-development/seongnam/'],
  ['seongnam-app-dev-quote', '/app-development/seongnam/'],
  ...
  ['seongnam-web-dev', '/web-development/seongnam/'],
  ['seongnam-homepage-dev', '/web-development/seongnam/'],
  ['seongnam-website-dev', '/web-development/seongnam/'],
  ['seongnam-landing-page', '/web-development/seongnam/'],
  ['seongnam-web-dev-cost', '/web-development/seongnam/'],
  ['seongnam-web-dev-quote', '/web-development/seongnam/'],
  ['seongnam-homepage-dev-cost', '/web-development/seongnam/'],
  ['seongnam-homepage-dev-quote', '/web-development/seongnam/'],
  ['seongnam-landing-page-cost', '/web-development/seongnam/'],
  ['seongnam-landing-page-quote', '/web-development/seongnam/'],
  // MVP → /mvp/{region}/
  ['suwon-mvp-dev', '/mvp/suwon/'],
  ['seongnam-mvp-dev', '/mvp/seongnam/'],
]);

/**
 * 지역명이 없는 service_intent 랜딩은 비용·가격·제작비·견적처럼 단어만 바꾼 96개 URL이었다.
 * 별도 질문이 아니라 같은 구매 의도이므로 강한 가이드 하나로 통합한다. 쇼핑몰·서비스기획은
 * 정확히 대응하는 현행 가이드가 없어 각각 cost 대표 랜딩 하나만 남긴다.
 */
const SERVICE_INTENT_CANONICAL = new Map([
  ['app-dev', '/guide/app-cost/'],
  ['app-dev-out', '/guide/app-cost/'],
  ['mobile-app', '/guide/app-cost/'],
  ['homepage-dev', '/guide/web-cost/'],
  ['web-dev', '/guide/web-cost/'],
  ['web-dev-simple', '/guide/web-cost/'],
  ['landing-page', '/guide/web-cost/'],
  ['website-dev', '/guide/web-cost/'],
  ['responsive-web', '/guide/web-cost/'],
  ['mvp-dev', '/guide/mvp-cost/'],
  ['shopping-mall', '/l/shopping-mall-cost/'],
  ['service-plan', '/l/service-plan-cost/'],
]);
const RETAINED_SERVICE_INTENT = new Set(['shopping-mall-cost', 'service-plan-cost']);
for (const landing of landingsData) {
  if (landing.pattern !== 'service_intent' || RETAINED_SERVICE_INTENT.has(landing.slug)) continue;
  const target = SERVICE_INTENT_CANONICAL.get(landing.serviceKey);
  if (target) REDIRECTED_LANDING_SLUGS.set(landing.slug, target);
}
