...
export interface PageSeo {
  ...
  /**
   * 검색 의도가 갈리는 지점에서 다른 서비스 페이지로 보내는 문맥 링크.
   *
   * sections 의 body 는 순수 텍스트로 렌더되어 링크를 넣을 수 없다. 그렇다고 본문에
   * 키워드만 적어 두면, 다른 것을 찾아온 방문자가 갈 곳을 못 찾고 이탈한다.
   * (예: /ai-development/ 로 들어왔지만 실제로 필요한 것은 사내 문서 검색인 경우)
   * label 은 링크 대상의 주제를 그대로 쓴다 — "자세히 보기" 류의 앵커를 쓰지 않는다.
   */
  crossLinks?: { heading: string; note: string; href: string; label: string }[];
}
...
export const PAGE_SEO_MAP: Record<string, PageSeo> = {
  ...
  '': {
    ...
    description:
      '앱·웹·AI MVP를 기획부터 개발·배포까지 제작합니다. 정액 견적, 소스코드와 운영 권한 이관, 전국 비대면 상담을 제공합니다.',
    keywords: [
      '외주개발',
      '앱개발 외주',
      'Flutter 앱개발',
    ...
    ogTitle: '앱·웹·AI 외주개발 | MVP 기획·배포·소스 이관 — 름랩',
    ogDescription:
      '앱·웹·AI MVP를 기획부터 개발·배포까지 제작합니다. 정액 견적, 소스코드와 운영 권한 이관, 전국 비대면 상담을 제공합니다.',
  ...
  'app-development': {
    ...
    sections: [
      ...
      { h2: '기술 선택 이유', body: 'Flutter는 하나의 코드베이스로 iOS·Android를 함께 만들어, 플랫폼별로 같은 기능을 두 번 구현하는 작업과 공통 수정의 중복을 줄일 수 있습니다. 절감 폭은 네이티브 기능·외부 SDK·테스트 범위에 따라 달라집니다. 관리자 웹은 Next.js로 만들어 검색 노출과 페이지 속도 기본기를 함께 확보합니다. 기술을 늘어놓기보다, 만들려는 서비스에 그 선택이 맞는지 상담에서 먼저 판단합니다.' },
  ...
  mvp: {
    ...
    whyPoints: [
      ...
      'VAT 포함 정액(580만 원부터)으로 가격을 먼저 공개하고, 패키지 밖 범위는 계약 전 별도 안내',
    ...
    sections: [
      ...
      { h2: 'MVP에 넣을 것 vs 빼야 할 것', body: '검증 목표(투자·영업·사용자 테스트)에 직접 필요한 기능만 넣습니다. 회원·결제·관리자 중 검증에 꼭 필요한 흐름만 남기고, 부가 기능·예외 처리·확장 옵션은 검증 후로 미룹니다. 기능을 늘리면 화면·테스트·연동 범위가 커져 비용과 기간도 달라지므로, 후속 버전으로 미룰 항목을 계약 전에 구분합니다.' },
  ...
  flutter: {
    ...
    description:
      'Flutter 하나의 코드베이스로 iOS·Android 앱을 함께 개발합니다. 공통 기능과 수정의 중복을 줄일 수 있지만, 절감 폭은 네이티브 연동과 테스트 범위에 따라 달라집니다. VAT 포함 580만 원부터, 소스코드 전체 이관 + 직접 운영 교육. 동탄·수원 거점, 전국 어디서나 진행.',
    ...
    faqs: [
      { q: 'Flutter로 만들면 네이티브보다 품질이 떨어지나요?', a: '일반적인 UI·회원·예약·결제·데이터 중심 앱에는 Flutter가 적합할 수 있습니다. 다만 고사양 3D, 낮은 지연이 중요한 실시간 영상, 특정 기기 SDK에 크게 의존하는 기능은 네이티브가 더 적합할 수 있어 견적 전에 기술 적합성을 확인합니다.' },
  ...
  'ai-development': {
    title: 'AI 외주개발 | RAG·챗봇·업무자동화 구축 — 름랩',
    description:
    ...
    sections: [
      ...
      { h2: '포함 범위와 별도 확인 범위', body: '기본 범위는 합의한 AI 기능, 기존 서비스 연동, 답변 범위·사람 이관 규칙, 테스트, 소스코드와 운영 안내입니다. 원본 데이터 정리, 맞춤 모델 학습, 대량 문서 정제, 외부 AI·서버 사용료는 자료와 사용량을 확인해 별도 범위로 정합니다. 실제 운영 데이터가 없는 단계에서 정확도나 업무시간 절감 수치를 보장하지 않습니다.' },
    ],
    faqs: [
      ...
      { q: '사내 자료를 AI가 참고하게 할 수 있나요?', a: '자료의 형태에 따라 다릅니다. 항목이 일정하게 정리된 문서·엑셀은 불러와 활용할 수 있고, 형식이 제각각이면 정리 기준부터 잡아야 합니다. 실제 파일을 보고 가능한 범위와 필요한 준비를 먼저 알려 드립니다. 사내 문서 검색과 질의응답이 주 목적이라면 사내 AI 구축 페이지에서 구조와 비용 기준을 따로 정리해 두었습니다.' },
      { q: '맞춤형 AI 모델 학습도 기본 범위에 포함되나요?', a: '기본은 검증된 외부 AI API를 업무 흐름에 연결하고 답변 범위와 검수 절차를 설계하는 방식입니다. 별도 모델 학습이 필요한지는 데이터 양·품질·보안 조건을 확인한 뒤 구분하며, 학습 데이터 정리와 모델 운영 비용은 자동으로 포함되지 않습니다.' },
    ],
    // 이 페이지는 "고객 응대용 챗봇" 축이다. 사내 문서 검색을 찾아온 방문자가 여기 머물면
    // 둘 다 손해라 명시적으로 길을 나눠 준다(자기잠식 방지 + 이탈 방지).
    crossLinks: [
      {
        heading: '사내 문서로 답하는 AI를 찾으신다면',
        note: '고객 응대용 챗봇이 아니라, 사내 규정·매뉴얼·업무 자료를 검색해 임직원 질문에 답하는 AI를 찾고 계신다면 성격이 다른 작업입니다. 검색·질의응답(RAG) 구조와 권한 설계를 따로 다룹니다.',
        href: '/enterprise-ai/',
        label: '사내 AI 구축 알아보기',
      },
      // 이 페이지는 "텍스트로 주고받는 챗봇" 축이다. 전화를 AI가 받게 하려는 방문자는
      // 음성·실시간·회선이라는 다른 조건을 안고 오므로 여기서 길을 나눠 준다(자기잠식 방지).
      // 이 페이지는 "AI 기능 하나를 붙이는" 축이다. 업무 자체를 끝까지 맡기려는 방문자는
      // 시스템 접근 권한과 예외 처리라는 다른 조건을 안고 오므로 여기서 길을 나눠 준다.
      {
        heading: '업무 자체를 AI가 처리하게 하려면',
        note: '답변을 만드는 AI가 아니라, ERP·CRM·문서·메일에 직접 들어가 조회하고 입력하고 회신까지 끝내는 AI가 필요하다면 접근 권한과 예외 처리 설계가 더해집니다. 구축 방식과 비용 기준이 달라 따로 다룹니다.',
        href: '/ai-worker/',
        label: 'AI Worker 구축 보기',
      },
      {
  ...
  'source-handover': {
    title: '소스코드 이관 외주 | 저장소·배포 권한 인수 조건 — 름랩 REUMLAB',
    description:
      '앱·웹 외주의 계약 범위 소스코드·GitHub 저장소와 이전 가능한 배포·운영 권한을 인수할 조건을 안내합니다. 제3자 라이선스와 플랫폼 계정 정책은 별도로 확인합니다. 이미 외주사에서 소스코드를 못 받은 경우 현황 점검·이관 상담도 진행합니다.',
    keywords: [
      '소스코드 이관',
      '소스코드 주는 외주',
      '외주 소스코드',
    ...
    ogDescription:
      '계약 범위의 소스코드·저장소와 이전 가능한 배포·운영 권한을 인수하는 조건을 확인하세요.',
    ...
    funnel: '전환 직전',
    serviceDesc:
      '름랩은 프로젝트 종료 시 계약 범위의 소스코드와 GitHub 저장소, 배포 설정·문서, 이전 가능한 스토어·서버 권한을 정리해 넘깁니다. 제3자 라이선스와 플랫폼 정책으로 양도가 불가능한 항목은 계약 전에 구분합니다. 이미 다른 외주사에서 소스코드를 못 받았거나 일부만 받은 경우는 보유 자료·계약·계정 권한을 먼저 확인해 이관 가능 범위를 안내합니다.',
    whyPoints: [
      '계약 범위 소스코드·GitHub 저장소·배포 설정과 이전 가능한 운영 권한 이관',
      '외주사 종속 없이 다른 개발사 이전·직접 운영·유지보수 가능',
      '정액 패키지에 기본 포함 — 별도 비용 없음',
      '외주사에서 못 받은 경우 현황 점검·이관 가능성 상담',
    ],
    sections: [
      { h2: '소스코드를 꼭 받아야 하는 이유', body: '소스코드가 없으면 작은 수정도 기존 외주사에 의존하게 되고, 연락이 끊기면 운영과 수정이 중단될 위험이 있습니다. 계약 범위의 소스코드·저장소·배포 정보를 보유하면 다른 개발사로 옮기거나 내부 팀이 이어받기 쉬워집니다. 이관 조건을 계약에서 확인하지 않는 것은 외주 위험의 대표적인 원인 중 하나입니다.' },
      { h2: '무엇을 이관받나 (이관 범위)', body: '계약 범위의 소스코드와 GitHub 저장소, 배포 설정·실행 문서, 이전 가능한 서버·도메인·스토어 권한을 정리합니다. 제3자 소프트웨어·폰트·이미지 라이선스와 플랫폼이 양도를 허용하지 않는 계정은 소유권 자체를 보장하지 않고, 가능한 이전·재가입 방법을 별도로 안내합니다.' },
      ...
      { h2: '름랩의 이관 방식', body: '름랩은 “만들고 끝”이 아니라 “운영 가능한 상태로 넘김”을 원칙으로 합니다. 납품 시 계약 범위의 소스코드·저장소와 이전 가능한 운영 권한을 정리해 이관하고, 제3자 라이선스나 플랫폼 정책상 이전이 제한되는 항목은 별도로 안내합니다. 비개발자 대표도 간단한 수정은 AI 도구로 직접 하도록 1:1 교육을 제공합니다.' },
    ],
    faqs: [
      { q: '소스코드 이관은 추가 비용인가요?', a: '아니요. 름랩 정액 패키지에는 소스코드·저장소·배포 권한 이관이 기본 포함입니다. 별도 비용이 없습니다.' },
      { q: '계약서에 소유권을 명시할 수 있나요?', a: '네. 소스코드·저장소·계정별 이관 범위를 계약 단계에서 명시합니다. 다만 제3자 라이선스나 플랫폼 정책으로 양도가 제한되는 항목은 예외와 대체 절차를 함께 적습니다.' },
  ...
  'admin-page-development': {
    title: '관리자 페이지 개발 | 주문·회원·재고·정산 자동화 — 름랩',
    description:
      '주문·회원·재고·정산·권한 관리를 한 화면으로 통합하는 관리자 페이지를 맞춤 개발합니다. 기존 서비스 연동과 소스 이관을 포함합니다.',
    keywords: [
      '관리자 페이지 개발',
      '어드민 페이지 개발',
      '백오피스 개발',
    ...
    sections: [
      ...
      { h2: '구축 범위와 별도 확인 항목', body: '계약 범위에는 합의한 관리자 화면, 역할별 권한, 데이터 입출력, 테스트, 소스코드와 실행 문서를 적습니다. 기존 데이터의 중복·오류 정리, 외부 서비스가 제공하지 않는 연동, 오래된 시스템 복구, 대규모 과거 데이터 이관은 원본과 접근 권한을 확인한 뒤 별도 범위로 산정합니다.' },
    ],
    faqs: [
      ...
      { q: '엑셀로 관리하던 데이터를 옮길 수 있나요?', a: '열 이름과 값 형식이 일정한 엑셀·CSV라면 이관할 수 있습니다. 파일마다 항목이 다르거나 중복·오류가 많으면 정리 기준과 별도 작업량을 먼저 확인합니다. 실제 파일을 본 뒤 자동 이관 범위와 수동 확인 항목을 구분해 안내합니다.' },
      ...
      { q: '기존 시스템 연동은 모두 가능한가요?', a: '상대 시스템의 API와 계정 접근 권한이 있어야 안정적으로 연동할 수 있습니다. API가 없거나 이용 조건이 제한되면 엑셀·CSV 교환 같은 대안을 검토하고, 확인되지 않은 연동 가능 여부를 계약 전에 약속하지 않습니다.' },
    ],
  ...
  maintenance: {
    title: '앱·웹 유지보수 업체 | 타사 소스 인수·오류 수정 — 름랩',
    description:
      '타사에서 만든 앱·웹도 소스 점검 후 오류 수정과 기능 개선을 이어받습니다. 월 계약 없이 범위와 비용을 먼저 확정합니다.',
    keywords: [
      '앱 유지보수',
      '웹 유지보수',
      '기존 앱 수정',
  ...
  'app-agency': {
    title: '앱개발 업체 비교 | 소스코드 이관·정액 견적 — 름랩',
    description:
      '앱개발 업체 선택 기준과 견적·소스코드·배포 권한·유지보수 조건을 비교하세요. Flutter 정액 개발과 전체 이관을 제공합니다.',
    keywords: [
      '앱개발 업체',
      '앱개발 회사',
      '앱 제작 업체',
