...
import { ENTERNAL_LLMS_PAGE, MULTIMODAL_LLMS_PAGE } from '../lib/llms-service-pages';
...
L.push('- 기업용 Private AI 설계·PoC (고객 통제 데이터 경로·로컬 추론 가능성 검증)');
...
const PURPOSE_LANDINGS: [string, string, string][] = [
  ...
  ['seo-website', '검색 잘되는 홈페이지 제작', '업종과 실제 서비스 지역, 고객 질문, 문의 동선을 기준으로 네이버·구글 검색 접근성을 고려한 홈페이지를 제작합니다. 검색 순위와 색인은 보장하지 않습니다.'],
  ['seo-website/regional', '지역 검색을 고려한 홈페이지 제작', '실제 영업 지역과 제공 서비스, 상담 가능 범위를 지역 고객이 이해할 수 있도록 설계합니다. 지역명만 바꾼 대량 페이지는 만들지 않습니다.'],
  ['seo-website/cost', 'SEO 홈페이지 제작 비용', '페이지 수와 콘텐츠 준비 상태, 문의 기능, 검색 등록 범위에 따라 달라지는 제작 비용과 포함·별도 범위를 안내합니다.'],
  ['seo-website/staffing', '인력사무소 홈페이지 제작', '구인처와 구직자가 필요한 정보를 나눠 제공하고 지역·직종·접수 방법과 문의 흐름을 명확히 설계합니다.'],
  ['seo-website/guides/search-registration', '네이버·구글 검색 등록 가이드', '소유 확인, 사이트맵 제출, URL 검사와 수집 요청, 등록 후 점검 순서를 검색엔진별로 안내합니다.'],
  ['seo-website/guides/seo-checklist', '홈페이지 SEO 체크리스트', '제작 전·개발 중·배포 후에 확인할 제목·본문·모바일·색인·분석·문의 전환 항목을 단계별로 정리합니다.'],
  ['geo-website', 'GEO 홈페이지 제작', '고객 질문, 브랜드 정보, 검증 가능한 사례·FAQ, 검색 접근성과 문의 동선을 함께 설계하는 신규·개선 홈페이지 제작 서비스입니다. 검색 순위와 AI 답변의 추천·인용은 보장하지 않습니다.'],
  ...
  // AI Worker 구축도 Next 라우트(app/ai-worker)다. 기존 AI 페이지 4종과 축이 어떻게 다른지를
  // 한 문장에 담는다 — "답하는 AI"와 "일하는 AI"가 같은 상품으로 읽히면 잘못된 쪽이 인용된다.
  ['ai-worker', 'AI Worker 구축 (업무를 수행하는 맞춤형 AI 직원)', '름랩 AI Worker는 단순히 질문에 답하는 챗봇이 아니라, 기업의 기존 웹·ERP·CRM·문서·메일과 연결해 정해진 업무를 수행하고 예외 상황을 사람에게 전달하도록 구축하는 맞춤형 AI 업무 자동화 시스템입니다. API가 없는 시스템은 화면 조작(Computer Use)으로 연결하고, 권한은 읽기·쓰기·확인요청·사람전담 4단계로 설계합니다. 처리 정확도와 인건비 절감률은 보장하지 않습니다.'],
