...
const PRICING = {
  'web-starter': { name: '웹 스타터', tag: '소상공인 소개·문의용 랜딩', price: '980,000', term: '약 5일', incl: ['원페이지 랜딩 · 모바일 반응형', '문의·예약 CTA 연결', '소스코드 이관 · AI 수정 교육 1회'], note: '별도: 멀티페이지·복잡한 관리자' },
  'web-marketing': { name: '웹 + 강력 마케팅', tag: '검색·광고로 문의 늘리기', price: '1,960,000', term: '약 10일', featured: true, incl: ['전환 카피 설계 + 기본 SEO', 'GA·픽셀 세팅 + 광고 소재 1세트', '소스코드 이관 · AI 수정 교육 1회'], note: '별도: 유료 광고비 · 멀티페이지 확장' },
  'web-business': { name: '웹 비즈니스', tag: '멀티페이지·블로그 정식 웹', price: '3,800,000', term: '약 14일', incl: ['멀티페이지(5p 내외) + 블로그', '예약·문의 + 간단 CMS · 기본 SEO', '소스코드 이관 · AI 운영 1:1 교육'], note: '별도: 결제·회원 등 웹앱 기능' },
  'web-premium': { name: '웹 프리미엄', tag: '관리자·외부 연동 웹앱', price: '5,800,000', term: '약 21일', incl: ['고도화 웹앱 + 관리자 페이지', '외부 연동(결제·지도·메일 1~2종)', '소스코드 이관 · AI 운영 교육'], note: '별도: 앱 · 대규모 연동 · AI 기능' },
  'app-light': { name: '앱 라이트 MVP', tag: '투자·테스트용 앱 빠른 검증', price: '5,800,000', term: '약 14일', incl: ['핵심 화면 3~5개 · 기본 데이터 연동', '문의·예약 흐름', '소스코드 이관 · AI 운영 1:1 교육'], note: '별도: 결제·회원 등 스탠다드 기능' },
  'app-standard': { name: '앱 스탠다드', tag: '회원·예약·결제 초기 서비스', price: '9,800,000', term: '약 21일', featured: true, incl: ['회원/로그인 + DB + 결제 또는 예약', '기본 관리자 + 실행·수정 가이드', '소스코드 이관 · AI 운영 1:1 교육'], note: '별도: AI 기능 · 대규모 연동' },
  'app-ai': { name: '앱 AI', tag: 'AI로 운영 자동화하는 서비스', price: '13,800,000', term: '약 30일', incl: ['스탠다드 + AI 기능 1종(챗봇·추천·요약)', '업무 자동화 협의', '소스코드 이관 · AI 운영 교육'], note: '별도: AI 사용량(API) 실비 · 고도화' },
 ...
 * 목적별 랜딩 FAQ.
 *
 * 이전에는 8개 랜딩이 FAQ_COMMON 하나를 그대로 출력해 질문·답변이 글자까지 같았다.
 * 소스코드 이관·계정 명의·추가 비용 같은 "회사 공통" 질문은 홈(index.html) FAQ 12문항이
 * 이미 담당하므로, 여기서는 각 목적에서만 나오는 질문만 둔다(§공통은 메인, 서비스는 특화).
 *
 * 답변 원칙
 *  - 첫 문장에서 바로 답한다(answer-first). 회사 홍보로 시작하지 않는다.
 *  - 가격·기간·유지보수 기간·계약 조건은 사이트에 이미 공개된 범위 안에서만 구체적으로 쓴다.
 *    공개되지 않은 숫자는 만들지 않고 "무엇이 그 값을 정하는지"로 답한다.
 *  - AI 성능을 과장하지 않는다(100% 정확·완전 자동화·사람 불필요 금지).
 *
 * key = 랜딩 slug. 없으면 FAQ_FALLBACK 을 쓴다.
 */
const FAQ_BY_SLUG = {
  mvp: [
    { q: 'MVP 개발이 일반 앱·웹 개발과 무엇이 다른가요?', a: '검증에 필요한 핵심 기능만 먼저 만들어 출시하는 방식입니다. 모든 기능을 갖춘 뒤 반응을 보는 대신, 확인하려는 것 하나에 필요한 흐름부터 내놓습니다. 초기 비용과 실패 위험이 줄고, 반응을 본 뒤 같은 코드 위에서 확장합니다.' },
    { q: 'MVP에 어떤 기능을 넣어야 하나요?', a: '무엇을 검증할지(투자·영업·사용자 테스트)부터 정하고 거기에 직접 필요한 흐름만 남깁니다. 회원·결제·관리자 중에서도 검증에 필요한 것만 넣고, 부가 기능과 예외 처리는 뒤로 미룹니다. 어떤 기능이 필수인지는 상담에서 함께 추립니다.' },
    { q: 'MVP 개발 기간은 어떻게 정해지나요?', a: '화면 수, 회원·결제 유무, 관리자 기능 범위, 외부 연동, 스토어 심사 여부가 일정을 정합니다. 름랩 기준으로 앱 라이트는 약 14일, 회원·결제·관리자까지 갖춘 앱 스탠다드는 약 21일입니다. 투자·데모 일정이 정해져 있으면 그 날짜에서 역산해 범위를 잡습니다.' },
    { q: '디자인도 함께 진행되나요?', a: '네. 화면 흐름 설계와 UI 디자인을 개발과 함께 진행합니다. 보유하신 로고·컬러·폰트 같은 브랜드 자료가 있으면 반영이 빨라집니다.' },
    { q: 'MVP 출시 후 기능을 추가할 수 있나요?', a: '가능합니다. 소스코드와 저장소를 통째로 이관하므로 같은 코드 위에서 단계적으로 얹습니다. 내부 개발팀이 생기면 그대로 이어받을 수 있고, 확장을 름랩에 다시 맡기실 수도 있습니다.' },
  ],
  erp: [
    { q: '맞춤형 ERP는 기존 ERP 솔루션과 무엇이 다른가요?', a: '회사의 업무 방식에 맞춰 화면과 흐름을 직접 설계한다는 점이 다릅니다. 기성 솔루션은 정해진 기능에 업무를 맞춰야 하지만, 맞춤 구축은 지금 쓰는 순서 그대로 만듭니다. 다만 표준 업무만 쓰신다면 기성 솔루션이 더 경제적일 수 있어, 그런 경우 그렇게 말씀드립니다.' },
    { q: '엑셀로 관리하던 업무를 시스템으로 옮길 수 있나요?', a: '가능합니다. 쓰시던 엑셀·CSV를 불러와 이어서 쓰도록 마이그레이션을 설계에 포함합니다. 다만 파일마다 항목이 제각각이면 정리 기준부터 잡아야 해서, 실제 파일을 보고 가능한 범위를 먼저 알려 드립니다.' },
    { q: '직원별로 볼 수 있는 화면을 다르게 할 수 있나요?', a: '네. 관리자·매니저·직원처럼 역할을 나누고 역할별 접근 권한과 노출 화면을 다르게 설계합니다. 정산·회원정보처럼 민감한 항목은 권한이 있는 계정만 보도록 구성합니다.' },
    { q: '지금 쓰는 다른 시스템과 연동할 수 있나요?', a: '상대 시스템이 API를 제공하는지에 따라 달라집니다. 현재 쓰시는 도구를 확인해 연동 가능한 범위와, 어려울 경우의 대안(주기적 엑셀·CSV 주고받기 등)을 먼저 정리해 드립니다.' },
    { q: '모바일에서도 쓸 수 있나요?', a: '가능합니다. 현장에서 확인·입력이 필요한 화면은 모바일·태블릿에서 쓰도록 반응형으로 설계하거나, 필요하면 별도 앱으로 분리합니다. 어떤 기기에서 주로 쓸지 미리 알려 주시면 그 화면을 기준으로 잡습니다.' },
  ],
  'ai-automation': [
    { q: '정해진 흐름이 아니라 AI가 알아서 판단하게 만들 수도 있나요?', a: '요청마다 필요한 도구와 순서가 달라지는 구조를 AI Agent 라고 부릅니다. 도구를 연결하고 승인 흐름을 넣은 실행형 구조는 설계·구현이 가능한 범위지만, 여러 도구를 AI 가 스스로 골라 다단계로 실행하는 시스템을 납품한 실적은 아직 없습니다. 원하시는 동작을 먼저 듣고 확실히 만들 수 있는 범위를 구분해 말씀드립니다. 대부분의 사내 업무는 정해진 흐름과 승인 절차로 충분하고 그 편이 더 안정적입니다.' },
    { q: '어떤 업무를 AI로 자동화할 수 있나요?', a: '같은 일이 반복되고 판단 기준이 비교적 뚜렷한 업무가 적합합니다. 반복 문의 응답, 접수 내용 분류, 문서 요약, 조건에 맞는 항목 추천 등이 대표적입니다. 반대로 줄일 업무가 뚜렷하지 않으면 상담에서 그렇게 말씀드리고 권하지 않습니다.' },
    { q: 'ChatGPT를 연결하는 것과 AI 시스템 개발은 무엇이 다른가요?', a: '어디까지 자동으로 처리하고 어디서 사람에게 넘길지를 업무 흐름에 맞춰 설계한다는 점이 다릅니다. 답할 범위와 데이터, 결과 확인 방법을 정해 실제 운영에 넣는 작업이 대부분을 차지합니다.' },
    { q: '기존 앱·웹에 AI 기능만 추가할 수 있나요?', a: '가능합니다. 이미 운영 중인 서비스를 다시 만들지 않고 필요한 기능만 붙이는 방식으로 진행합니다. 현재 구조를 보고 연동 가능한 범위를 먼저 확인합니다.' },
    { q: 'AI 사용 비용은 어떻게 발생하나요?', a: '개발비와 별개로, AI 제공사에 내는 사용량 기반 요금이 실비로 발생합니다. API 키는 대표님 명의로 보유하도록 이관해 사용량과 비용을 직접 확인하실 수 있고, 사용량을 줄이는 방법도 함께 안내합니다.' },
    { q: 'AI가 잘못된 답을 하면 어떻게 하나요?', a: 'AI는 틀릴 수 있다는 전제로 설계합니다. 답할 범위를 미리 정해 두고 벗어나는 질문은 사람에게 넘기며, 확정되지 않은 가격·일정처럼 잘못 답하면 곤란한 항목은 자동 응답에서 제외합니다. 어디까지 맡길지는 운영 방식을 듣고 함께 정합니다.' },
    { q: '맞춤형 AI 모델 학습도 기본 범위인가요?', a: '기본은 검증된 외부 AI API를 업무 흐름에 연결하고 입력·출력·사람 검수 기준을 설계하는 방식입니다. 별도 모델 학습이 필요한지는 보유 데이터의 양·품질·보안 조건을 확인한 뒤 판단하며, 데이터 정리와 모델 운영 비용은 자동으로 포함되지 않습니다.' },
  ],
  platform: [
    { q: '플랫폼 개발은 일반 앱 개발과 무엇이 다른가요?', a: '사용자가 두 종류 이상이라는 점이 가장 큰 차이입니다. 수요자·공급자·운영자가 각각 다른 화면을 쓰고 그 사이에서 매칭·정산·분쟁 처리가 오가기 때문에, 화면 수보다 흐름 설계에 시간이 더 듭니다.' },
    { q: '중개 수수료와 정산도 구현할 수 있나요?', a: '가능합니다. 국내 PG를 연동해 결제를 받고, 수수료 계산과 정산 내역 집계를 관리자에서 다루도록 설계합니다. 다만 정산 규칙이 복잡할수록 범위가 커지므로 규칙을 먼저 정리한 뒤 견적을 잡습니다.' },
    { q: '처음부터 모든 기능을 만들어야 하나요?', a: '아닙니다. 플랫폼은 양쪽 사용자가 모두 모여야 돌아가기 때문에, 한쪽 흐름부터 열어 검증한 뒤 확장하는 편이 안전합니다. 무엇부터 열지는 사업 계획을 듣고 함께 정합니다.' },
    { q: '개발 비용은 어떻게 정해지나요?', a: '사용자 유형 수, 화면 수, 매칭·정산 로직의 복잡도, 결제·외부 API 연동, 실시간 기능 유무로 정해집니다. 름랩은 VAT 포함 정액으로 계약 전에 금액을 공개하며, 범위 안에서는 진행 중 추가 비용이 없습니다.' },
    { q: '사용자가 늘어나면 감당할 수 있나요?', a: '초기부터 확장을 염두에 둔 구조로 만들되, 처음부터 대규모 트래픽 설계를 하지는 않습니다. 실제 사용량이 보이면 그 시점의 데이터를 기준으로 개선하는 편이 비용 면에서 합리적입니다.' },
  ],
  'reservation-commerce': [
    { q: '예약과 결제를 한 흐름으로 묶을 수 있나요?', a: '네. 예약 신청부터 결제, 확정 알림, 취소·환불까지 하나의 흐름으로 설계합니다. 국내 PG를 연동해 카드·간편결제를 받고, 운영 규칙에 맞춰 취소·환불 처리를 반영합니다.' },
    { q: '노쇼를 줄이는 기능도 넣을 수 있나요?', a: '가능합니다. 예약금 결제, 예약 확정·리마인드 알림, 취소 기한 규칙 같은 장치를 운영 방식에 맞춰 조합합니다. 어떤 조합이 맞는지는 업종과 취소 패턴을 듣고 정합니다.' },
    { q: '이미 쓰는 예약 서비스가 있는데 새로 만들어야 하나요?', a: '꼭 그렇지는 않습니다. 지금 도구로 충분하면 홈페이지에서 연결만 하는 편이 낫고, 정원·차수·회원 등급처럼 기성 도구가 막는 규칙이 있을 때 맞춤 구축이 유리합니다. 현재 방식을 먼저 확인하고 판단해 드립니다.' },
    { q: '회원 등급이나 정기 결제도 가능한가요?', a: '가능합니다. 회원 등급별 가격·혜택, 정기권·구독 결제, 잔여 횟수 관리 등을 운영 규칙에 맞춰 구현합니다. 규칙이 복잡할수록 범위가 커지므로 상담에서 먼저 정리합니다.' },
    { q: '매출과 정산 내역을 볼 수 있나요?', a: '네. 예약·결제·취소·환불 내역과 기간별 매출을 관리자에서 집계해 보고 엑셀로 내려받을 수 있도록 구성합니다. 어떤 기준으로 보고 싶은지 알려 주시면 그 화면으로 만듭니다.' },
  ],
  website: [
    { q: '홈페이지 제작과 웹 서비스 개발은 무엇이 다른가요?', a: '홈페이지는 정보를 보여 주고 문의로 이어지게 하는 것이 목적이고, 웹 서비스는 회원·결제·데이터 처리처럼 사용자가 무언가를 하는 기능이 중심입니다. 름랩은 둘 다 진행하며, 목적에 따라 필요한 범위를 먼저 정합니다.' },
    { q: '검색에 잘 나오게 만들어 주나요?', a: '메타 태그·구조화 데이터·사이트맵·모바일 반응형·페이지 속도 같은 기본기를 제작에 포함합니다. 다만 순위를 보장하지는 않습니다. 네이버 플레이스·구글 비즈니스 프로필 등록과 함께 가야 효과가 나므로 그 순서도 안내해 드립니다.' },
    { q: '완성 후 제가 직접 수정할 수 있나요?', a: '네. 연락처·가격·문구·이미지·블로그처럼 자주 바뀌는 항목은 관리 화면에서 직접 수정하도록 만들고 사용법을 1:1로 알려 드립니다. 월 관리비는 없습니다.' },
    { q: '제작 비용은 어떻게 정해지나요?', a: '페이지 수, 예약·회원·결제 같은 기능 유무, 관리자 기능 범위, 기존 사이트 이전 여부로 정해집니다. 원페이지 랜딩은 VAT 포함 98만 원부터, 멀티페이지에 CMS·블로그를 갖춘 구성은 380만 원이 기준이며 계약 전에 금액을 공개합니다.' },
    { q: '기존 홈페이지를 옮기면 검색 순위가 떨어지나요?', a: '기존 URL과 콘텐츠를 보존하고 주소가 바뀌는 페이지에 301 리다이렉트를 설계하면 손실을 최소화할 수 있습니다. 오히려 모바일·속도·구조화 데이터 개선으로 노출이 좋아지는 경우가 많습니다.' },
  ],
  'data-seo': [
    { q: '페이지를 많이 만들면 검색에 유리한가요?', a: '아닙니다. 내용이 비슷한 페이지를 대량으로 찍어 내면 오히려 도어웨이로 판단돼 색인에서 빠집니다. 페이지마다 고유한 정보가 있고 검색 의도가 다를 때만 늘리는 것이 의미가 있습니다.' },
    { q: '저품질 페이지가 양산되는 것을 어떻게 막나요?', a: '페이지별 고유 정보 확보, 검색 의도에 맞는 콘텐츠, canonical·중복 제거, 내부 링크 클러스터링, 색인 상태 관리를 함께 설계합니다. 기준에 못 미치는 페이지는 색인에서 제외하는 게이트를 두는 방식도 씁니다.' },
    { q: '검색 순위를 보장해 주나요?', a: '보장하지 않습니다. 순위는 검색엔진이 정하고 경쟁 상황에 따라 달라집니다. 름랩이 하는 일은 크롤링·색인·구조화 데이터·내부 링크 같은 기술 기반을 정확히 갖추는 것까지입니다.' },
    { q: '네이버와 구글을 함께 대응할 수 있나요?', a: '가능합니다. 사이트맵·robots·구조화 데이터 같은 공통 기반을 갖추고, 네이버 서치어드바이저 등록과 RSS 같은 채널을 함께 정리합니다. 다만 두 엔진의 노출 방식이 달라 결과가 같지는 않습니다.' },
    { q: '작업 후 효과를 어떻게 확인하나요?', a: '구글 서치콘솔·네이버 서치어드바이저의 색인 수와 검색 노출·클릭, GA4의 유입과 문의 전환을 기준으로 봅니다. 검색 결과는 반영에 시간이 걸려 보통 몇 주 단위로 확인합니다.' },
  ],
  'service-renewal': [
    { q: '다른 업체가 만든 서비스도 이어받을 수 있나요?', a: '네. 소스코드와 저장소가 있으면 구조를 점검한 뒤 수정·개선을 이어받습니다. 없으면 보유 자료와 권한 관계로 어디까지 이관·복구가 가능한지부터 확인하고 시작합니다.' },
    { q: '소스코드를 못 받았는데 고칠 수 있나요?', a: '보유하신 자료와 계약·권한 관계에 따라 다릅니다. 현황을 점검해 복구·이관 가능 범위를 알려 드리고, 복구가 어려우면 기존 데이터를 살려 다시 만드는 방안도 함께 검토합니다.' },
    { q: '먼저 무엇부터 확인하나요?', a: '넘겨받은 코드가 실제로 빌드되고 배포 가능한 상태인지부터 확인합니다. 저장소·배포 권한·계정이 있는지, 구조가 이어받을 만한지를 진단해 가능 범위와 위험을 먼저 알려 드립니다.' },
    { q: '전부 다시 만들어야 하나요?', a: '대부분은 아닙니다. 살릴 수 있는 부분은 살리고 막힌 지점만 고치는 편이 비용·기간 면에서 유리합니다. 다시 만드는 편이 나은 경우에는 그 이유와 예상 범위를 함께 설명드립니다.' },
    { q: '오래돼 스토어에서 오류가 나는 앱도 되나요?', a: '가능합니다. OS·스토어 정책 변경으로 생긴 빌드·심사 오류 대응은 인수 유지보수에서 흔한 작업입니다. 점검 후 수정 범위와 일정을 안내해 다시 정상 배포되도록 진행합니다.' },
  ],
};

/** FAQ_BY_SLUG 에 없는 랜딩용 최소 공통 세트 */
const FAQ_FALLBACK = [
...
const LANDINGS = [
  ...
  },
  {
    ...
    bespoke: [
      ...
      { title: '계약 전에 나누는 범위', type: 'scope', rows: [['기본 확인', '자동화할 업무 · 입력 데이터 · 출력 결과 · 사람 검수 · 오류 시 중단/이관'], ['계약 범위', '합의한 자동화 단계 · 연동 · 테스트 · 소스코드 · 운영 문서'], ['별도 협의', '원본 데이터 정리 · 맞춤 모델 학습 · 외부 API 사용료 · 대규모 레거시 연동']] },
      ...
      // 이 페이지는 "정해진 업무 흐름"을 자동화하는 축이다. 요청마다 써야 할 도구와
      // 순서가 달라지는 경우는 Agent 구조 검토 대상이라, 그 설명이 있는 곳으로 보낸다.
      { title: '요청마다 흐름이 달라진다면', type: 'linknote', text: '자동화는 정해진 업무 흐름을 그대로 실행하는 구조입니다. 요청마다 필요한 도구와 순서가 달라져 고정 흐름으로 담기 어렵다면 AI Agent 구조를 검토할 수 있습니다. 경계와 실제 제공 범위를 정리해 두었습니다.', href: '/enterprise-ai/#ai-agent', linkLabel: 'AI Agent 구조 확인하기' },
    ],
  ...
  },
  {
    ...
    bespoke: [
      ...
      { title: '참여자 흐름', type: 'flow', steps: ['고객 검색·요청', '견적·매칭', '수락·결제', '진행', '완료', '리뷰·정산'] },
  ...
  },
  {
    ...
    bespoke: [
      ...
      { title: '결제·취소 정책을 먼저 정합니다', type: 'list', items: ['결제 시점(예약 시 / 방문 시)', '취소·환불 규칙과 기한', '노쇼(no-show) 처리 방식', '부분 환불·수수료 기준'] },
  ...
  },
  {
    ...
    bespoke: [
      ...
      { title: '저품질 페이지를 만들지 않기 위해', type: 'list', items: ['페이지별 고유 정보 확보', '검색 의도에 맞는 콘텐츠', 'canonical·중복 제거', '내부 링크 클러스터링', '색인 상태 지속 관리'] },
      { title: '일반 업체 홈페이지가 필요하다면', type: 'linknote', text: '대규모 데이터 수집·자동화가 아니라 업종과 서비스 지역을 설명하고 문의를 받는 홈페이지가 목적이라면 제작 범위가 다릅니다.', href: '/seo-website/', linkLabel: '검색 잘되는 홈페이지 제작 범위 확인하기' },
...
function faqsFor(land) {
  return FAQ_BY_SLUG[land.slug] || FAQ_FALLBACK;
}

function faqSection(land) {
  const items = faqsFor(land).map((f) => `      <div class="faq-item"><button class="faq-q"><span class="qm">Q</span>${esc(f.q)}<svg class="chev" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4"><path d="M6 9l6 6 6-6" stroke-linecap="round" stroke-linejoin="round"/></svg></button><div class="faq-a"><div class="faq-a__in">${esc(f.a)}</div></div></div>`).join('\n');
...
function jsonLd(land) {
  ...
  const graph = {
    ...
    '@graph': [
      ...
      { '@type': 'FAQPage', '@id': `${url}#faq`, mainEntity: faqsFor(land).map((f) => ({ '@type': 'Question', name: f.q, acceptedAnswer: { '@type': 'Answer', text: f.a } })) },
    ],
  };
