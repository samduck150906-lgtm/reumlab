...
test('빌드 산출물 검증기는 완전한 계약만 통과시키고 로고·스키마·제품 단계 누락을 보고한다', () => {
  ...
  const wordmarkFile = join(route, 'enternal-ai-wordmark.png');
  ...
  const validHtml = `<!doctype html><html><head><title>Enternal AI | 기업용 Private AI·사내 AI PoC | 름랩</title><meta name="description" content="기업 데이터를 고객 환경 안에서 활용하는 Private AI를 목표로 개발합니다. 로컬 추론·사내 문서 연결·자체 사전학습 기반의 적용 범위를 기업별 PoC로 검증합니다."><meta name="robots" content="index,follow"><link rel="canonical" href="${ENTERNAL_CANONICAL}"><script type="application/ld+json">${schema}</script></head><body><h1>기업의 모든 지식이 기업 안에서 더 똑똑해집니다.</h1><p>현재 제공</p><p>PoC 검증</p><p>개발 방향</p><p>목표 구조</p><input name="유입_랜딩" value="/enternal-ai/">${flowMarkers}${comparisonMarkers}${faqMarkers}</body></html>`;

  function writeLogo(file: string, width: number, height: number) {
  ...
  try {
    ...
    writeLogo(logoFile, 2172, 724);
    writeLogo(wordmarkFile, 432, 144);
    ...
    writeLogo(logoFile, 2172, 724);
    rmSync(wordmarkFile);
    assert.ok(verifyEnternalArtifacts(root).some((error) => error.includes('wordmark')));
