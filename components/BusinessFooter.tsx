    ...
    <footer className="footer">
      <div className="container">
        ...
        <nav className="footer-info" aria-label="주요 서비스" style={{ marginBottom: 14, display: 'flex', flexWrap: 'wrap', gap: '8px 16px' }}>
          ...
          <Link href="/ai-voice-development/">AI 음성상담·전화 자동화</Link>
          ...
          <Link href="/seo-website/">검색 잘되는 홈페이지</Link>
          <Link href="/geo-website/">GEO 홈페이지 제작</Link>
        ...
        */}
        ...
        <nav aria-label="공식 채널" style={{ marginTop: 10, display: 'flex', flexWrap: 'wrap', gap: '6px 14px', fontSize: 13 }}>
          {SITE.socialProfiles.map((profile) => (
            <a key={profile.url} href={profile.url} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--text-dim)' }}>
              {profile.label}
            </a>
          ))}
        </nav>
